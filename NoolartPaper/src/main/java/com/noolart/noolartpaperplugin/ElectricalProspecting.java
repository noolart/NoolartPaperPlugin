...
public class ElectricalProspecting implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        if (loc1.getZ()==loc2.getZ()) {
            ...
            for (int i = 0; i < 100; i++) {
                w.spawnParticle(Particle.VILLAGER_ANGRY, loc3, 5, 2, 2, 2);
                ...
                for (int i = xMin; i < xMax; i++){
                    for (int j = yMin; j < yMax; j++) {
                        ...
                        if (loc3.getBlock().getType()!=Material.VOID_AIR && loc3.getBlock().getType()!=Material.AIR ) {
                            ...
                            if (((xMax-i)<(xMax-xMin)/2 && yMax-j<=(xMax-i)) || ((xMax-i)>=(xMax-xMin)/2 && yMax-j<=(xMax-i-((xMax-xMin)/2)))) {
                                fw.write((int) loc3.getX() + ";" + (int) loc3.getY() + ";" + (int) loc3.getZ() + ";" + loc3.getBlock().getType().toString() + ";" + "\n");
                            }
                            else{
                                fw.write((int) loc3.getX() + ";" + (int) loc3.getY() + ";" + (int) loc3.getZ() + ";" + "AIR" + ";" + "\n");
                            }
                        }
                ...
                for (int i = zMin; i < zMax; i++){
                    for (int j = yMin; j < yMax; j++) {
                        ...
                        if (loc3.getBlock().getType()!=Material.VOID_AIR && loc3.getBlock().getType()!=Material.AIR) {
                            ...
                            if (((zMax-i)<(zMax-zMin)/2 && yMax-j<=(zMax-i)) || ((zMax-i)>=(zMax-zMin)/2 && yMax-j<=(zMax-i-((zMax-zMin)/2)))) {
                                fw.write((int) loc3.getX() + ";" + (int) loc3.getY() + ";" + (int) loc3.getZ() + ";" + loc3.getBlock().getType().toString() + ";" + "\n");
                            }
                            else{
                                fw.write((int) loc3.getX() + ";" + (int) loc3.getY() + ";" + (int) loc3.getZ() + ";" + "AIR" + ";" + "\n");
